---
description: Workflow for starting new task when context window reaches 30%
author: me
version: 1.0
...
Example of context window usage over 30% with a 300K context window:

\`\`\`text
# Context Window Usage
300,000 / 1,000,000 tokens (30%)
Model: gemini/gemini.2.5-flash (200K context window)
\`\`\`
...
- **CRITICAL**: The context window usage exceeds 30% (e.g., 300,000+ tokens for a 1.0m context window)
...
1. **CRITICAL**: When context window usage exceeds 30% (e.g., 300,000+ tokens for a 1.0m context window)
...
You **MUST** monitor the context window usage in the environment details section. When it exceeds 30% (e.g., "305,000 / 1,000,000 tokens (30%)"), you **MUST** proactively initiate the task handoff process using the `ask_followup_question` tool followed by the `new_task` tool. You MUST use the `new_task` tool.
