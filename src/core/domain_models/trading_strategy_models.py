...
class DayTradingParameters(BaseModel):
    ...
    def slow_period_must_be_greater_than_fast(cls, v, info):
        ...
        if v is not None and 'macd_fast_period' in info.data and info.data['macd_fast_period'] is not None and v <= info.data['macd_fast_period']:
    ...
    def overbought_must_be_greater_than_oversold(cls, v, info):
        ...
        if v is not None and 'rsi_oversold' in info.data and info.data['rsi_oversold'] is not None and v <= info.data['rsi_oversold']:
...
class GridTradingParameters(BaseModel):
    ...
    def check_prices(self) -> 'GridTradingParameters':
    ...
    def validate_parameters_match_strategy_type(cls, v, info):
        ...
        if expected_model:
            ...
            if expected_model is Dict[str, Any]:
                # If the expected model is a generic dictionary, no further validation needed here
                pass
            elif isinstance(v, dict) and not isinstance(v, expected_model):
                try:
                    v = expected_model(**v)
            ...
            elif not isinstance(v, expected_model):
                # This case handles when 'v' is not a dict and not an instance of the expected model
                # This implies a type mismatch that Pydantic's Union should have caught,
                # but we re-raise for clarity if it somehow reaches here.
                raise ValueError(
                    f"Parameters type mismatch for strategy type '{strategy_type_enum_member.value}'. "
                    f"Expected {expected_model.__name__}, got {type(v).__name__}."
                )
