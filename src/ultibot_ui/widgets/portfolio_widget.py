...
class PortfolioWidget(QWidget):
    ...
    def _handle_update_result(self, result_data: Dict[str, Any]):
        try:
            ...
            if self.current_capital_status:
            ...
            QMessageBox.warning(self, "Portfolio Update Error", f"Hubo un error al procesar los datos del portafolio.\nDetalles: {error_msg}")
        finally:
    ...
    def _update_capital_alerts(self, capital_status: Dict[str, Any]):
        ...
        if alerts:
            ...
            if any("LÍMITE DIARIO ALCANZADO" in alert for alert in alerts):
                self.capital_alerts_frame.setStyleSheet("QFrame { border: 2px solid #dc3545; border-radius: 5px; background-color: #3a3a3a; padding: 10px; }")
            elif any("CERCA DEL LÍMITE" in alert for alert in alerts):
                self.capital_alerts_frame.setStyleSheet("QFrame { border: 2px solid #ffc107; border-radius: 5px; background-color: #3a3a3a; padding: 10px; }")
        else:
            self.capital_alerts_frame.setVisible(False)
    ...
    def _handle_worker_error(self, error_msg: str):
        ...
        QMessageBox.critical(self, "Error de Datos del Portafolio", f"No se pudieron cargar los datos del portafolio.\nDetalles: {error_msg}\n\nPor favor, intente actualizar manualmente o revise la conexión.")
