...
class MockUserConfigPersistence:
    ...
    async def get_user_configuration(self, user_id: str) -> Optional[Any]:
        return self._user_configs.get(user_id)

    async def _real_upsert_user_configuration(self, config: Any):
        self._user_configs[str(getattr(config, "user_id", None))] = config
        return config
...
@pytest_asyncio.fixture
async def persistence_service_fixture(db_session: AsyncSession) -> AsyncMock:
    """Fixture que proporciona un mock del servicio de persistencia."""
...
async def configuration_service_fixture(
    credential_service_fixture: AsyncMock,
    mock_notification_service_fixture: AsyncMock,
    persistence_service_fixture: AsyncMock
) -> AsyncMock:
    """Fixture para ConfigurationService con dependencias mockeadas."""
    mock = AsyncMock(spec=ConfigurationService)
    # Configura los métodos del mock si es necesario
    return mock
...
def trade_factory():
    ...
    """
    import json
    from core.domain_models.orm_models import TradeORM
    from core.domain_models.trade_models import (
...
async def market_data_service_fixture(
    credential_service_fixture: AsyncMock,
    mock_binance_adapter_fixture: AsyncMock,
    persistence_service_fixture: AsyncMock
) -> AsyncMock:
    """Fixture para MarketDataService con dependencias mockeadas."""
    mock = AsyncMock(spec=MarketDataService)
    # Configura los métodos del mock si es necesario
    return mock

@pytest_asyncio.fixture
async def portfolio_service_fixture(
    market_data_service_fixture: AsyncMock,
    persistence_service_fixture: AsyncMock
) -> AsyncMock:
    """Fixture para PortfolioService con dependencias mockeadas."""
...
async def unified_order_execution_service_fixture():
    ...
    async def mock_execute_market_order(
        ...
        oco_order_list_id: Optional[str] = None # Removed price, added oco_order_list_id
    ) -> TradeOrderDetails:
        print(f"DEBUG: Mock execute_market_order called with: user_id={user_id}, symbol={symbol}, side={side}, quantity={quantity}, trading_mode={trading_mode}, api_key={api_key}, api_secret={api_secret}, oco_order_list_id={oco_order_list_id}")
        ...
        return TradeOrderDetails(
            ...
            type="MARKET",
            status=OrderStatus.FILLED.value,
            requestedPrice=None, # Set to None as price is not passed to mock
            ...
            ocoOrderListId=oco_order_list_id, # Use the passed oco_order_list_id
            price=None, # Set to None
            stopPrice=None,
            timeInForce=None,
        )
    ...
    async def mock_create_oco_order(
        user_id: UUID,
        symbol: str,
        side: TradeSide, # Changed from str to TradeSide
        quantity: Decimal,
        price: Decimal,
        stop_price: Decimal,
        limit_price: Decimal,
    ...
    # Configurar el mock para el método de validación
    mock_service.validate_trading_mode.return_value = True
    
    return mock_service

@pytest_asyncio.fixture
async def ai_orchestrator_fixture(market_data_service_fixture: AsyncMock) -> AsyncMock:
    """Fixture para AIOrchestrator con mock."""
    mock = AsyncMock(spec=AIOrchestrator)
    # Configura los métodos del mock si es necesario
    return mock
...
async def trading_engine_fixture(
    ...
    credential_service_fixture: AsyncMock,
    mock_notification_service_fixture: AsyncMock,
    mock_strategy_service_integration: AsyncMock,
    configuration_service_fixture: AsyncMock,
    portfolio_service_fixture: AsyncMock,
    ai_orchestrator_fixture: AsyncMock
) -> AsyncMock:
    """Fixture completa para TradingEngine con todas las dependencias mockeadas."""
    mock = AsyncMock(spec=TradingEngine)
    # Configura los métodos del mock si es necesario
    return mock
