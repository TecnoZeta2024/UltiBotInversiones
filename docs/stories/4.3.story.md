...
- [ ] **Subtask 1: Backend - Endpoint de Confirmación de Operación Real** (AC: 1, 3)
    - [ ] 1.1: Definir un nuevo modelo Pydantic para la solicitud de confirmación de operación real (ej. `ConfirmRealTradeRequest`) que incluya el `opportunity_id` y cualquier otro dato necesario para la confirmación.
    - [ ] 1.2: Crear un nuevo endpoint `POST /api/v1/trading/real/confirm-opportunity/{opportunity_id}` en `src/ultibot_backend/api/v1/endpoints/trading.py` (o un nuevo archivo si no existe `trading.py`) que reciba la confirmación del usuario.
    - [ ] 1.3: Implementar la lógica en el endpoint para validar que la oportunidad existe, que su estado es `pending_user_confirmation_real`, que el modo real está activo y que hay cupos disponibles (consultando `ConfigService`).
    - [ ] 1.4: Si las validaciones son exitosas, llamar al `TradingEngineService` para iniciar la ejecución de la orden real.
    - [ ] 1.5: Manejar posibles errores (ej. oportunidad no encontrada, modo real inactivo, sin cupos) devolviendo respuestas HTTP apropiadas (ej. 400 Bad Request, 404 Not Found).

- [ ] **Subtask 2: Backend - Lógica de Ejecución de Orden Real en `TradingEngineService`** (AC: 3, 5, 6)
    - [ ] 2.1: En `src/ultibot_backend/services/trading_engine_service.py`, implementar un método asíncrono (ej. `execute_real_trade`) que reciba el `opportunity_id` y el `user_id`.
    - [ ] 2.2: Dentro de `execute_real_trade`, recuperar la `Opportunity` completa de la base de datos.
    - [ ] 2.3: Calcular la cantidad a operar (`requestedQuantity`) basándose en las reglas de gestión de capital (`UserConfiguration.riskProfileSettings` y `UserConfiguration.realTradingSettings`) y el saldo real disponible (obtenido de `BinanceAdapter` o `PortfolioService`).
    - [ ] 2.4: Utilizar el `BinanceAdapter` para enviar la orden real a Binance (`POST /api/v3/order`), asegurando el uso de las credenciales correctas (obtenidas de `CredentialService`).
    - [ ] 2.5: Registrar la orden enviada y su estado inicial en la base de datos (creando una nueva entidad `Trade` con `mode='real'` y `positionStatus='opening'`, incluyendo `entryOrder` de tipo `TradeOrderDetails`).
    - [ ] 2.6: Actualizar el `status` de la `Opportunity` a `converted_to_trade_real` y vincular el `trade_id` recién creado.
    - [ ] 2.7: Decrementar el `real_trades_executed_count` en `UserConfiguration.realTradingSettings` a través de `ConfigService`.
    ...
    - [ ] 2.9: Disparar notificaciones (a través de `NotificationService`) sobre el estado de la orden (ej. "Orden enviada", "Orden ejecutada", "Error al ejecutar").

- [ ] **Subtask 3: Frontend - Diálogo de Confirmación de Operación Real** (AC: 1, 2, 4)
    - [ ] 3.1: Crear un nuevo `QDialog` o `QWidget` en `src/ultibot_ui/widgets/` (ej. `real_trade_confirmation_dialog.py`) para mostrar los detalles de la oportunidad y solicitar confirmación.
    - [ ] 3.2: El diálogo debe recibir los datos de la `Opportunity` (par, dirección, confianza, resumen de IA, precio estimado de entrada, cantidad calculada) y presentarlos claramente.
    - [ ] 3.3: Incluir botones "Confirmar y Ejecutar Operación REAL" y "Cancelar".
    - [ ] 3.4: Al hacer clic en "Confirmar", llamar al nuevo endpoint de backend `POST /api/v1/trading/real/confirm-opportunity/{opportunity_id}` a través de `ApiClient`.
    - [ ] 3.5: Al hacer clic en "Cancelar", cerrar el diálogo y, opcionalmente, enviar una señal al backend para marcar la oportunidad como `rejected_by_user` o `simulated_post_facto`.
    - [ ] 3.6: Mostrar un indicador de carga mientras se espera la respuesta del backend.

- [ ] **Subtask 4: Frontend - Integración de la Confirmación en la UI** (AC: 1, 5)
    - [ ] 4.1: Modificar la vista de "Oportunidades" (creada en Historia 4.2) para que, al seleccionar una oportunidad `pending_user_confirmation_real`, se dispare la apertura del `real_trade_confirmation_dialog`.
    - [ ] 4.2: Implementar la lógica para que la UI refleje el estado de la orden después de la confirmación (ej. actualizar la lista de oportunidades, mostrar un mensaje de éxito/error).
    - [ ] 4.3: Asegurar que el contador de operaciones reales disponibles (AC4 de Historia 4.1) se actualice en la UI después de una ejecución exitosa.
